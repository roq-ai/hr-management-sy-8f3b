...
import { Mp3PlayerListPage as ShowCaseList } from 'pages/mp-3-players';

function DashboardPage() {
  const { session } = useSession();

  const favoritesStat = useSWR<PaginatedInterface<FavoritesInterface>>(
    () => `/favorites`,
    () => getFavorites({ limit: 1, offset: 0 }),
  );

  const mp3PlayerStat = useSWR<PaginatedInterface<Mp3PlayerInterface>>(
    () => `/mp-3-players`,
    () => getMp3Players({ limit: 1, offset: 0 }),
  );

  const playlistStat = useSWR<PaginatedInterface<PlaylistInterface>>(
    () => `/playlists`,
    () => getPlaylists({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Favorites',
      result: favoritesStat?.data?.totalCount,
      isLoading: favoritesStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Mp 3 Players',
      result: mp3PlayerStat?.data?.totalCount,
      isLoading: mp3PlayerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Playlists',
      result: playlistStat?.data?.totalCount,
      isLoading: playlistStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`mp-3-players`} passHref legacyBehavior>
                  <Button
